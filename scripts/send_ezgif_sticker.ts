...
async function sendEzgifStickerTest() {
    const accessToken = process.env.META_WA_ACCESS_TOKEN;
    const phoneNumberId = process.env.META_WA_PHONE_NUMBER_ID;
    const phone = '593963410409'; // César's phone

    const stickerFilePath = `C:\\Users\\Cesar\\Documents\\GRUPO EMPRESARIAL REYES\\PROYECTOS\\CRM OBJETIVO\\CRM V2\\stiker_perro_512.webp`;

    console.log('🐶 Iniciando envío de sticker OPTIMIZADO...');
    ...
    try {
        const stats = fs.statSync(stickerFilePath);
        ...
        formData.append('file', blob, 'stiker_perro_opt.webp');
