...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedLeads = localStorage.getItem('whatsapp_module_leads');
    if (savedLeads) {
      try {
      ...
      // Check for legacy key from original app if user runs it on same domain/port
      const legacy = localStorage.getItem('turismo_crm_leads');
      if (legacy) {
        try {
          setLeads(JSON.parse(legacy));
        } catch (e) {
          setLeads(parseCSV(CSV_RAW_DATA));
        }
      } else {
  ...
  useEffect(() => {
    if (dataLoaded && leads.length > 0) {
      localStorage.setItem('whatsapp_module_leads', JSON.stringify(leads));
    }
  }, [leads, dataLoaded]);
  ...
  return (
    <div className="flex h-[85vh] bg-gray-100 font-sans rounded-xl overflow-hidden shadow-2xl border border-gray-200">
      {/* Sidebar */}
      <div className="w-64 bg-gray-900 text-white flex flex-col h-full shadow-lg z-10 overflow-y-auto shrink-0 border-r border-gray-800">
        <div className="p-6 border-b border-gray-800">
          <h1 className="text-2xl font-bold tracking-tight">Turismo CRM</h1>
          <p className="text-xs text-gray-400 mt-1">Gestión Local Segura</p>
        </div>
        <nav className="p-4 space-y-2 flex-1">
          <button
            onClick={() => setCurrentView('dashboard')}
            className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-md transition-colors ${currentView === 'dashboard' ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 text-left rounded-md transition-colors ${currentView === 'email' ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}
          >
      ...
      {/* Main Content */}
      <div className="flex-1 p-8 h-full overflow-hidden flex flex-col">
